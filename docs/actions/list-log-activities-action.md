...
### ⚠️ Filament 5 - Panel Context Automatico

**IMPORTANTE**: Questo progetto utilizza **Filament 5**, NON Filament 3.

In Filament 5, quando si chiama `Resource::getUrl()` da un contesto Livewire (come `ListRecords`), Filament **determina automaticamente il panel corretto** dal contesto del livewire. Non è necessario passare esplicitamente il panel.

**✅ CORRETTO (Filament 5)**:
```php
// Filament 5 determina automaticamente il panel dal contesto
return $resource::getUrl('log-activity', ['record' => $record]);
```

**❌ ERRATO**:
```php
// NON esiste Filament\Support\Facades\Filament::getCurrentPanel() in Filament 5
$panel = Filament::getCurrentPanel(); // ❌ CLASSE NON ESISTENTE
return $resource::getUrl('log-activity', ['record' => $record], panel: $panelId);
```

Vedi [Filament 5 Important Notes](../../../../../../docs/filament4-important.md) per dettagli completi.
