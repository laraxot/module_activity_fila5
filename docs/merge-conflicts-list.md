# Merge conflict markers — file list

Elenco dei file trovati contenenti marker di merge (<<<<<<<, >>>>>>>, .merge_file). Seguire la lista per assegnare task agli agenti.

## Lista (auto-generata)

- memory/MEMORY.md
- _bmad/bmm/4-implementation/bmad-create-story/stories/1-1-segnalazione-ui-ux-improvement.md
- laravel/Themes/Sixteen/public/manifest.json
- docs/wiki/concepts/no-docs-archive-rule.md
- docs/wiki/index.md
- docs/wiki/log.md
- laravel/Themes/Sixteen/docs/wiki/index.md
- laravel/Themes/Sixteen/docs/wiki/log.md
- laravel/Modules/Lang/docs/module-analysis.md
- laravel/Modules/Lang/docs/translatable/json-content-translation.md
- laravel/Modules/Lang/docs/module.md
- laravel/Modules/Notify/docs/wiki/index.md
- laravel/Modules/Geo/resources/js/components/coordinate-picker-styles.js
- laravel/Modules/Geo/docs/wiki/index.md
- laravel/Modules/Geo/docs/wiki/log.md
- laravel/Modules/Geo/docs/wiki/AGENTS.md
- laravel/Modules/Ptvx/docs/wiki/index.md
- laravel/Modules/Ptvx/docs/wiki/log.md
- laravel/Modules/Ptvx/docs/wiki/README.md
- laravel/Modules/Xot/docs/phpstan-fixes.md
- laravel/Modules/Ptvx/resources/views/filament/widgets/ticket-create-wizard.blade.php
- laravel/Modules/Xot/docs/phpstan-audit.md
- laravel/Modules/Xot/docs/filament-4-laraxot-e5a872.md
- laravel/Modules/Xot/docs/mcp-database-tools.md
- laravel/Modules/Xot/docs/laraxot.md
- laravel/Modules/Xot/docs/comprehensive-code.md
- laravel/Modules/Xot/docs/model-inheritance-audit.md
- laravel/Modules/Xot/docs/comprehensive-improvement-recommendations.md
- laravel/Modules/Xot/docs/file-structure.md
- laravel/Modules/Xot/docs/property-exists-elimination.md
- laravel/Modules/Xot/docs/moduleocumentation-standard.md
- laravel/Modules/Xot/docs/filament-installation-and-charts.md
- laravel/Modules/Xot/docs/property-exists-elimination-report.md
- laravel/Modules/Xot/docs/base-model.md
- laravel/Modules/Xot/docs/comprehensive-chart-pdf-guide.md
- laravel/Modules/Xot/docs/models/dry-kiss-analysis.md
- laravel/Modules/Ptvx/resources/views/filament/widgets/ticket-create-wizard.blade.php

## Stato

- [x] laravel/Modules/Ptvx/resources/views/filament/widgets/ticket-create-wizard.blade.php — risolto (merge markers rimossi e unione logica applicata)
- [x] laravel/Modules/Activity/docs/wiki/README.md — risolto (merge markers rimossi)
- [ ] gli altri file: da verificare e risolvere

## Note

- Regola: ogni modulo e tema deve avere un file `docs/merge-conflicts-list.md` con la lista locale e lo stato di avanzamento.
- Quando si risolve un file, aggiornare lo stato nella lista e sincronizzare l'LLM Wiki (qmd embed) per ingestione.